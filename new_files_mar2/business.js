...
var fannieMaeBizApp = new function() {
	...
	_this.getAlertData = function(page) {
		...
		if ( page == 'ADS' ) {
			...
			query = {"size":10,"query":{"match":{"data":"\"ReconType\":\"ADS2EDI\""}},"sort":[{"timestamp":{"order":"desc","mode":"avg"}}]};

		} else {
			url = urlObj.bizLDNGAlerts;

			query = {"size":10,"query":{"match":{"data":"\"ReconType\":\"LDNG2ADS\""}},"sort":[{"timestamp":{"order":"desc","mode":"avg"}}]};
		}
	...
	_this.createAlertTable = function(data, page) {
		...
		data.forEach(function (obj) {
			...
			var date = new Date(parseInt(o.timetsamp));
			var fields = [];
			var alertDes = o.AlertDescription != undefined ? o.AlertDescription : [];
